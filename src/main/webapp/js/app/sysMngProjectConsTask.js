...
docName = "系统管理-基础数据管理-项目建设任务信息";
mainObject = "sysMngProjectConsTask";
deleteObject = "sysMngProjectConsTask";
formId = "#projectConsTaskForm";
appName = "项目建设任务信息";
selectArray = ["#id", "#project","#engineeringType"];
...
$(function () {
    ...
    var grid = $(dataTableName).bootgrid({
        ...
        url: "/sysMngProjectConsTask/data",
        formatters: {
    ...
    $("#saveBtn").on("click", function () {
        //获取表单对象
        var bootstrapValidator = $("#projectConsTaskForm").data('bootstrapValidator');
        // //手动触发验证
        bootstrapValidator.validate();
        //如果验证成功就保存数据
        if(bootstrapValidator.isValid()){
            var obj = getFormJsonData("projectConsTaskForm");
            var object = JSON.parse(obj);
            console.log("obj-------------" + obj);
            var url = "sysMngProjectConsTask/save";
    ...
    $('#confirm_modal').on('hidden.bs.modal', function() {
        $("#projectConsTaskForm").data('bootstrapValidator').destroy();
        $('#projectConsTaskForm').data('bootstrapValidator', null);
        formValidator();
    });
...
/**
 * 用户选择多个项目建设任务信息，进行批量授权
 */
function authorize() {
    var selectedIds = getCheckedValues();
    if (!selectedIds) {
        showMessageBox("danger", "请选择要审核的项目建设任务信息！");
        return;
    }
    var url = "sysMngProjectConsTask/authorizeInBatch";
    var requestData = {
        projectConsTaskIds: selectedIds
    };
