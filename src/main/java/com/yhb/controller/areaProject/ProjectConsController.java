...
import com.yhb.service.areaProject.AreaProjectConsSearchService;
 ...
 */
@Controller
@RequestMapping("/projectCons")
public class ProjectConsController extends BaseController {
    //复合查询条件参数个数
    private final int paramsNum = 8;
    @Autowired
    AreaProjectConsSearchService areaProjectConsSearchService;
    ...
    public MyPage data(HttpServletRequest request,HttpSession session,
        ...
        return new PageUtils().searchBySortServiceWithSelectedIds(areaProjectConsSearchService, searchPhrase, paramsNum, current, rowCount, pageable);
    }
    ...
    public void exportExcel(HttpServletRequest request, HttpSession session, HttpServletResponse response, @RequestParam("param") String param,
        ...
        List<AreaProjectCons> dataList = areaProjectConsSearchService.findByConditions(param, paramsNum,selectedIds);
        areaProjectConsSearchService.setDataList(dataList);
        areaProjectConsSearchService.exportExcel(request, response, docName, titles, colNames);
    }
    ...
    public List<Long> selectAllIds() {
        return areaProjectConsSearchService.selectAllIds();
    }
