 ...
 */
@Controller
@RequestMapping("/projectConsTask")
public class ProjectConsTaskController extends BaseController {
    //复合查询条件参数个数
    private final int paramsNum = 5;
    @Autowired
    AreaProjectConsTaskSearchService areaProjectConsTaskSearchService;
    ...
    public MyPage data(HttpServletRequest request, HttpSession session,
        ...
        return new PageUtils().searchBySortServiceWithSelectedIds(areaProjectConsTaskSearchService, searchPhrase, paramsNum, current, rowCount, pageable);
    }
    ...
    public void exportExcel(HttpServletRequest request, HttpSession session, HttpServletResponse response, @RequestParam("param") String param,
        ...
        List<AreaProjectConsTask> dataList = areaProjectConsTaskSearchService.findByConditions(param, paramsNum,selectedIds);
        areaProjectConsTaskSearchService.setDataList(dataList);
        areaProjectConsTaskSearchService.exportExcel(request, response, docName, titles, colNames);
    }
    ...
    public List<Long> selectAllIds() {
        return areaProjectConsTaskSearchService.selectAllIds();
    }
