...
import com.yhb.service.log.AppAccessLogSearchService;
...
@Data
public class AppAccessLogController extends BaseController {
    //复合查询条件参数个数
    private final int paramsNum = 1;

    @Autowired
    AppAccessLogSearchService appAccessLogSearchService;

    String objectName = "应用访问日志信息";
    ...
    public MyPage data(HttpServletRequest request,
        ...
        return new PageUtils().searchBySortServiceWithSelectedIds(appAccessLogSearchService, searchPhrase, paramsNum, current, rowCount, pageable);
    }
    ...
    public void exportExcel(HttpServletRequest request, HttpServletResponse response, @RequestParam("param") String param, @RequestParam("docName") String docName,
        ...
        List<AppAccessLog> dataList = appAccessLogSearchService.findByConditions(param, paramsNum, selectedIds);
        appAccessLogSearchService.setDataList(dataList);
        appAccessLogSearchService.exportExcel(request, response, docName, titles, colNames);
    }
    ...
    public List<Long> selectAllIds() {
        return appAccessLogSearchService.selectAllIds();
    }
}
